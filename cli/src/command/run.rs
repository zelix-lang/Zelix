...
use shared::{logger::{Logger, LoggerImpl}, path::retrieve_path, result::try_unwrap};
use std::{env::current_dir, fs::{exists, metadata}};

use crate::structs::surf_config_file::SurfConfigFile;

pub fn run_command(path: Option<PathBuf>) {
    let final_path = retrieve_path(path.unwrap_or(
        try_unwrap(
            current_dir(), 
