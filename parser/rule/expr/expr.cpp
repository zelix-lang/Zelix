...
#include "expr.h"
#include "parser/rule/signed/signed.h"
...
template<bool Condition, bool ForLoop>
void parser::rule::expression(
    ast *&root,
...
)
{
    ...
    else
    {
        ...
        constexpr auto delimiter = Condition ? lexer::token::OPEN_CURLY // If condition, use open curly brace
    ...
    while (!expr_queue.empty())
    {
        ...
        while (first->type == lexer::token::AMPERSAND || // &
...
#       endif
        ...
        if (process_next(node, candidate, trace, first_opt,
                         first) // Process the next token
        )
            continue;

        if (likely & expr::CALL_LIKELY && first->type == lexer::token::OPEN_PAREN)
        {
            candidate = call(candidate, expr_stream, allocator, queue_allocator,
                             expr_queue); // Call the function with the candidate
        ...
        {
            candidate = prop(candidate, expr_stream, allocator, trace, queue_allocator,
                             expr_queue); // Call the property access with the candidate
        ...
        if (likely & expr::ARITHMETIC_OP_LIKELY &&
            (first->type == lexer::token::PLUS || first->type == lexer::token::MINUS ||
             first->type == lexer::token::MULTIPLY || first->type == lexer::token::DIVIDE))
        {
            candidate = signed_op(candidate, expr_stream, allocator, queue_allocator, expr_queue);

            first_opt = expr_stream.peek(); // Get the current token
            if (first_opt.is_none())
            {
                process_next(node, candidate, trace, first_opt,
                             first); // Process the next token
                continue;
            }
        ...
        if (likely & expr::BOOLEAN_OP_LIKELY &&
            (first->type == lexer::token::BOOL_EQ || first->type == lexer::token::BOOL_GT ||
             first->type == lexer::token::BOOL_GTE || first->type == lexer::token::BOOL_LT ||
             first->type == lexer::token::BOOL_LTE || first->type == lexer::token::BOOL_NEQ))
        {
            candidate = signed_op<false>(candidate, expr_stream, allocator, queue_allocator, expr_queue);
