...
void zelix::parser::rule::for_loop(
    ast *&root,
...
)
{
    ...
    auto range_group = extract(tokens, lexer::token::TO, lexer::token::UNKNOWN, lexer::token::UNKNOWN, false, false);
    ...
    rule::expression<false, true>(from_range_node, range_group, allocator,
                                  trace); // Parse the range expression
    ...
    while (next_opt.is_some())
    {
        ...
        if (next->type == lexer::token::STEP)
        {
            ...
            rule::expression<false, true>(queue_node, st, allocator,
                                          trace); // Parse the range expression
