...
#include "function.h"
#include "parser/rule/block/block.h"
#include "parser/rule/type/type.h"
#include "parser/expect.h"

void zelix::parser::rule::function(
    ast *&root,
...
)
{
    ...
    const auto name = tokens.next().get();
    ...
    else
    {
        ...
        while (true)
        {
            ...
            expect(tokens, lexer::token::IDENTIFIER);
            const auto arg = tokens.next().get();
    ...
    if (const auto type_peek = peek_opt.get(); type_peek->type == lexer::token::ARROW)
    {
