...
// MarshalSingleFunction marshals a single function into the given strings.Builder
func MarshalSingleFunction(
	...
	// Write the function's original name for debugging purposes
...
func MarshalFunctions(
	...
	for function, name := range functions {
		if function.IsInMod() {
			// Modules' functions are constructed later
			continue
		}

		MarshalSingleFunction(
			ir,
			function,
			name,
			fileCode,
